...
async def download_callback(current, total):
    current_readable = humanize.naturalsize(current)
    total_readable = humanize.naturalsize(total)

    print(f"Downloaded {current_readable} out of {total_readable} bytes")
...
<code>{download_time}</code>
    ...
    file = await event.client.send_file(
        event.chat_id,
        title,
        caption=msg, parse_mode='HTML', progress_callback=lambda current, total: download_callback(current, total)
)
    await yy.delete()
